...
The launcher's color is worth one more sentence, because it is where a phone shell would normally reach for a second palette. It does not have one. The app plates are tinted with the four semantic fills the theme already ships for badges, every glyph is full-strength ink on top, and exactly one plate wears the accent: the app that is the site's actual conversion. That decision has a consequence for retheming, and [CUSTOMIZE.md](CUSTOMIZE.md) shows it with two pictures, one of them a failure left in on purpose.
